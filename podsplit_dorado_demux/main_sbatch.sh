...
JOBID_05=$(sbatch --parsable --job-name=download_models --account=${CPU_ACCOUNT} --partition=${CPUJOB_PARTITION} --output="$LOG_DIR/%x-%A-%a.log" --error="$LOG_DIR/%x-%A-%a.log" ${SCRIPT_PATH}/download_models__sbatch_job.sh) 
...
JOBID_10=$(sbatch --parsable --dependency=afterok:${JOBID_05} --account=${CPU_ACCOUNT} --partition=${CPUJOB_PARTITION} --job-name=pod5_split --output="$LOG_DIR/%x-%A-%a.log" --error="$LOG_DIR/%x-%A-%a.log" ${SCRIPT_PATH}/pod5_split__sbatch_job.sh)
