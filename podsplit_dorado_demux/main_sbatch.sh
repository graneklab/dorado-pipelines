...
JOBID_10=$(sbatch --parsable --dependency=afterok:${JOBID_05} --partition=${CPUJOB_PARTITION} --job-name=pod5_split --output="$LOG_DIR/%x-%A-%a.log" --error="$LOG_DIR/%x-%A-%a.log" ${SCRIPT_PATH}/pod5_split__sbatch_job.sh)
