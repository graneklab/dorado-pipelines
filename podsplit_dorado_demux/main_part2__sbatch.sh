...
JOBID_20=$(sbatch --parsable --array=0-$FILE_COUNT --job-name=dorado_duplex --partition=${GPUJOB_PARTITION} --account=${GPU_ACCOUNT} --gpus=${GPUJOB_GPUS} --output="$LOG_DIR/%x-%A-%a.log" --error="$LOG_DIR/%x-%A-%a.log" ${SCRIPT_PATH}/dorado_duplex__sbatch_job.sh)
...
if [ -v KIT_NAME ]; then 
    ...
    JOBID_30=$(sbatch --parsable --dependency=afterok:${JOBID_20} --job-name=mergebams --partition=${CPUJOB_PARTITION} --account=${CPU_ACCOUNT} --output="$LOG_DIR/%x-%A-%a.log" --error="$LOG_DIR/%x-%A-%a.log" ${SCRIPT_PATH}/mergebams__sbatch_job.sh)
...
if [ -v GENERATE_FASTQ ] && [ "TRUE" == "${GENERATE_FASTQ}" ]; then 
    ...
    JOBID_40=$(sbatch --parsable --dependency=afterok:${JOBID_30} --job-name=main_part3 --partition=${CPUJOB_PARTITION} --account=${CPU_ACCOUNT} --output="$LOG_DIR/%x-%A-%a.log" --error="$LOG_DIR/%x-%A-%a.log" ${SCRIPT_PATH}/main_part3__sbatch.sh)
else 
