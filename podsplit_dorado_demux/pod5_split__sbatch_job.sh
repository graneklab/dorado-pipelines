...
sbatch --job-name=main_2 --account=${CPU_ACCOUNT} --partition=${CPUJOB_PARTITION} --output="$LOG_DIR/%x-%A-%a.log" --error="$LOG_DIR/%x-%A-%a.log"    ${SCRIPT_PATH}/main_part2__sbatch.sh 
